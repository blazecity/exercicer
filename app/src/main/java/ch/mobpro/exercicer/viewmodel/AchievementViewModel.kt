...
class AchievementViewModel @Inject constructor(private val repository: TrainingRepository): ViewModel()  {
    ...
    fun getSportSumsByDate(sportId: Long, fromDate: LocalDate, toDate: LocalDate): LiveData<AggregatedTraining> {
        return repository.getSportSumsByDate(sportId, fromDate, toDate).distinctUntilChanged().asLiveData()
    }

    fun getTrainingTypeSumsByDate(trainingTypeId: Long, fromDate: LocalDate, toDate: LocalDate): LiveData<AggregatedTraining> {
